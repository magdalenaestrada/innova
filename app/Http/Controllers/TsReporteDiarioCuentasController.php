...
class TsReporteDiarioCuentasController extends Controller
{
    ...
    public function show(Request $request, string $id)
    {
        ...
        $ingresos_error = $cuenta->ingresos->whereNull('fecha');

        $ingresos_error->each(function ($item) {
            if ($item->created_at) {
                $item->fecha = \Carbon\Carbon::parse($item->created_at)->format('Y-m-d');
                $item->save(); // Persist the change
            }
        });


        $salidas_error = $cuenta->salidas->whereNull('fecha');

        $salidas_error->each(function ($item) {
            if ($item->created_at) {
                $item->fecha = \Carbon\Carbon::parse($item->created_at)->format('Y-m-d');
                $item->save(); // Persist the change
            }
        ...
        $startOfToday = now()->startOfDay(); // Equivalent to 12:00 AM today
        

        // Calculate saldo_anterior (sum of montos before today's start time)
       $saldo_anterior = $cuenta->ingresos
        ...
        // Filter ingresos and salidas to only show records from today 12:00 AM onwards
         $ingresos = $cuenta->ingresos->where('fecha', '>=', $startOfToday)->sortBy(['fecha', 'created_at']);
        $salidas = $cuenta->salidas->where('fecha', '>=', $startOfToday)->sortBy(['fecha', 'created_at']);
        ...
        return view('tesoreria.cuentas.reportes.show', compact( 'cuenta', 'ingresos', 'salidas', 'user', 'ultimo_cierre', 'saldo_anterior'));
    }
    ...
    public function filter(Request $request, string $id)
    {
        ...
        return view('tesoreria.cuentas.reportes.show', compact('cuenta_general', 'cuenta', 'ingresos', 'salidas', 'user', 'saldo_anterior'));
    }
