...
class TsReposicioncajaController extends Controller
{
    ...
    public function index()
    {
        ...
        $reposicionescajas = TsReposicioncaja::with(['tipocomprobante', 'caja', 'motivo', 'creador', 'salidacuenta'])
        ->orderBy(
            TsSalidaCuenta::select('fecha')
                ->whereColumn('id', 'ts_reposiciones_cajas.salida_cuenta_id')
                ->latest() // Ensure the latest record is used
                ->take(1),
            'desc'
        )
        ->paginate(30);
