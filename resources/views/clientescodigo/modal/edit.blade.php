...
<div class="modal fade text-left" id="ModalEdit{{ $cliente->id }}" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="card-body">
                <form class="editar-empleado" action="{{ route('clientescodigo.editar', $cliente->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <select name="tipo_documento" class="form-control">
                                <option value="1" {{ $cliente->tipo_documento == 1 ? 'selected' : '' }}>DNI
                                </option>
                                <option value="2" {{ $cliente->tipo_documento == 2 ? 'selected' : '' }}>RUC
                                </option>
                            </select>
                        ...
                        <div class="form mb-1 col-md-3">
                            ...
                            <input name="documento" id="documento-{{ $cliente->id }}" value="{{ $cliente->documento }}"
                                class="form-control" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="11"
                                oninput="this.value = this.value.replace(/[^0-9]/g, '')" required>
                        </div>

                        <div class="form col-md-6">
                            <label>Nombre</label>
                            <input name="nombre" id="nombre-{{ $cliente->id }}" value="{{ $cliente->nombre }}"
